        ...
        if values["prophet_param"] == "custom":
            ...
            values["custom_param"] = {
                ...
                "fourier_order": st.number_input(
                    f"Fourier order for {seasonality} seasonality", value=15
                ),
                "prior_scale": st.number_input(
                    f"Prior scale for {seasonality} seasonality", value=10
                ),
            }
...
def input_prior_scale_params(config: dict) -> dict:
    ...
    seasonality_prior_scale = st.number_input(
        "seasonality_prior_scale", value=default_params["seasonality_prior_scale"]
    )
    holidays_prior_scale = st.number_input(
        "holidays_prior_scale", value=default_params["holidays_prior_scale"]
    )
    changepoint_prior_scale = st.number_input(
        "changepoint_prior_scale", value=default_params["changepoint_prior_scale"], format="%.3f"
    )
    params["prior_scale"] = {
...
def input_other_params(config: dict, params: dict) -> dict:
    ...
    changepoint_range = st.number_input(
        "changepoint_range", value=default_params["changepoint_range"], format="%.2f"
    )
    params["other"] = {
        "growth": growth,
...
def input_regressors(df: pd.DataFrame, config: dict, params: dict) -> dict:
    ...
    if len(eligible_cols) > 0:
        ...
        regressor_cols = st.multiselect(
            "Select external regressors if any", list(eligible_cols), default=default_regressors
        )
        for col in regressor_cols:
            regressors[col] = dict()
            regressors[col]["prior_scale"] = st.number_input(
                f"Prior scale for {col}", value=default_params["regressors_prior_scale"]
            )
            regressors[col]["mode"] = st.selectbox(
                f"Mode for {col}", default_params["seasonality_mode"]
            )
    else:
