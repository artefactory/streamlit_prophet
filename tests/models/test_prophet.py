...
from tests.samples.dict import make_resampling_test, make_cleaning_test, make_dates_test, make_params_test
...
def test_forecast_workflow(use_cv, make_future_forecast):
    df = df_test[20]
    params = make_params_test(regressors={col: {'prior_scale': 10, 'mode': 'additive'}
    ...
    datasets, models, forecasts = forecast_workflow(config, use_cv, make_future_forecast, cleaning,
                                                    resampling, params, dates, datasets)
    assert all([len(set(['ds', 'yhat']).intersection(set(x.columns))) == 2 for x in forecasts.values()])
    ...
    if use_cv:
        ...
        assert datasets['eval'].ds.nunique() == datasets['train'].ds.nunique() + datasets['val'].ds.nunique()
